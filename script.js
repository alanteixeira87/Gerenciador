...
const app = {
    ...
    checkLogin: function() {
        ...
        const SENHA_B64 = window.QA_LOCAL_CONFIG?.passwordBase64;

        if (!SENHA_B64) {
            error.textContent = "Configuração local de acesso não encontrada.";
    ...
    populateLists: function() {
        const createOptions = (arr, elId) => {
            ...
            [...new Set(arr)]
                ...
                .forEach(i => { const opt = document.createElement('option'); opt.value = i; el.appendChild(opt); });
        };
    ...
    setupListeners: function() {
        ...
        const logTest = document.getElementById('logTeste');
        if (logTest) {
            logTest.addEventListener('input', () => {
                clearTimeout(this.logTestInputTimer);
                this.logTestInputTimer = setTimeout(() => this.updateLongDurationTestSelect(), 160);
            });
            logTest.addEventListener('change', () => {
                clearTimeout(this.logTestInputTimer);
                this.updateLongDurationTestSelect();
            });
        }
    },
...
const chicago = {
    ...
    generate: function(type) {
        ...
        switch(type) {
            ...
            case 'e2e_pf':
            case 'payments_v5_open_pf':
            case 'payments_v4_scheduling_pf':
            case 'payments_v5_scheduling_pf':
                payload.resource = {
                    loggedUserIdentification: d.cpf,
                    creditorAccountIspb: "22896431",
                    creditorAccountIssuer: "0001",
            ...
            case 'redirect_pj': 
            case 'e2e_pj':
            case 'payments_v5_open_pj':
            case 'payments_v4_scheduling_pj':
            case 'payments_v5_scheduling_pj':
                payload.resource = {
                    loggedUserIdentification: d.cpf,
                    businessEntityIdentification: d.cnpj,
                    creditorAccountIspb: "22896431",
